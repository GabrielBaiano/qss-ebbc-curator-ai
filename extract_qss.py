...
def clean_abstract_for_search(text):
    return text.lower()
...
def main():
    print("Iniciando extração de artigos do Quantitative Science Studies (QSS) Volume 6...")
    
    # 1. Parâmetros de busca no OpenAlex
    # ISSN do QSS: 2641-3337, Volume 6
    issn = "2641-3337"
    volume = "6"
    ...
    try:
        ...
        print(f"Erro ao conectar com a API do OpenAlex: {e}")
        return

    results = data.get("results", [])
    print(f"Total de artigos retornados pelo OpenAlex: {len(results)}")
    ...
    for i, w in enumerate(results, 1):
        ...
        item = {
            ...
            "_raw_abstract": abstract  # Guardamos temporariamente para conferência e refinamento manual refinado
        }
        
        json_list.append(item)
        
    # Gravar arquivo intermediário para análise e refinamento
    with open("qss_raw_data.json", "w", encoding="utf-8") as f:
        json.dump(json_list, f, ensure_ascii=False, indent=2)
        
    print("Concluído! Dados extraídos salvos em 'qss_raw_data.json'.")
